...
function fromPromise(source) {
  ...
  return new Readable({
    ...
    read: function () {
      ...
      // Emit an error on the stream if the source is not a promise
      if (
        ...
        process.nextTick(function () {
          self.emit('error', new TypeError('Expected `source` to be a promise.'));
        });
      ...
      // Listen to the promise and push results. We can call then repeatedly because
      // that's part of the promise spec. Presumably this would fail for bad promise
      // implementations, but we don't need to support those right?
      source.then(
